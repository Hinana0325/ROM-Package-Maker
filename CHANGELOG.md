...
- **升级 .NET 8 → .NET 10**：三个工程（主应用 / `_selftest` / `_realtest`）目标框架升级为 `net10.0-windows10.0.26100.0` / `net10.0`；`ZipSigner` 改用 `X509CertificateLoader` 加载 PFX（消除 .NET 10 新增的 `SYSLIB0057` 过时警告）；CI 的 `dotnet-version` 更新为 10.0.x。构建 0 警告 0 错误，自测 159 项全部通过

- **Windows App SDK 2.2.0 → 2.4.0**：NuGet 升级至最新稳定版（本机已安装 Windows App Runtime 2.4.0.0；自包含部署下运行时随应用分发）。构建 0 警告 0 错误，自测 159 项全部通过，GUI 实机启动验证正常（历史 `REGDB_E_CLASSNOTREG` 崩溃未复现）

- **界面视觉重构（品牌设计系统）**：
  - 自定义品牌配色（科技蓝紫 #4F6EF7 → #7B5CF7），浅色 / 深色双主题完整适配
  - 导航栏深色化（#1B1C22），菜单项浅色文字，选中项品牌色高亮 + 指示条
  - 主页 Hero 头部：品牌色渐变背景 + 白色标题 + 半透明状态条
  - 快速操作磁贴：品牌色浅底图标 + 标题 + 副标题，hover 上浮反馈
  - 工作流程：品牌色编号徽章 + 步骤间连接线
  - 全局卡片圆角 14、主按钮品牌色填充、内容区浅灰背景形成层次
  - 所有功能页通过全局样式自动升级
- **布局空间利用率优化**：
  - 主页：Hero 紧凑化，快速操作与工作流程改为左右分栏（左 2/3 磁贴+提示，右 1/3 流程），去掉 1000px 限宽
